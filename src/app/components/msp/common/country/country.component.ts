...
export class MspCountryComponent extends BaseComponent {
  ...
  countryData: Array<{ code: string; name: string }> = this.lang(
    './en/index.js'
  ).countryData;
  ...
  ngAfterViewInit(): void {
    this.form.valueChanges
      ...
      .subscribe(values => {
        const res = this.countryData.filter(itm => itm.name === this.country);
        if (this.country && this.country.length > 0 && res.length > 0) {
  ...
  isValid(): boolean {
    ...
    if (this.country && this.country.trim().length > 0 && res.length > 0) {
      return true;
    } else {
      return false;
    }
