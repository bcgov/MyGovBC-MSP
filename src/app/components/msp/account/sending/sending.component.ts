  ...
  transmitRequest() {
    ...
    this.service
      ...
      .then((mspAccountApp: MspAccountApp) => {
        ...
        this.logService.log(
          {
            name: 'Account - Received refNo ',
            confirmationNumber: this.mspAccountApp.referenceNumber
          },
          'Account - Submission Response Success '
        );
        ...
        this.router.navigate(['/msp/account/confirmation'], {
          queryParams: {
            confirmationNum: tempRef,
            showDepMsg: bcServicesCardElgible
          }
        });
      })
      .catch((error: ResponseType | any) => {
        ...
        console.log(
          'EA uuid updated: from %s to %s',
          oldUUID,
          this.dataService.getMspAccountApp().uuid
        );
