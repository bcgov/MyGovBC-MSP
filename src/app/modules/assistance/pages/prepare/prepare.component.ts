...
export class AssistancePrepareComponent
  ...
  @ViewChild('assistanceYearComp')
  assistanceYearComp: MspAssistanceYearComponent;

  @ViewChild('mspConsentModal') mspConsentModal: ConsentModalComponent;
  @ViewChild('disabilityNursingHomeChoiceModal')
  public disabilityNursingHomeChoiceModal: ModalDirective;
  ...
  CREDIT_CLAIM_CATEGORY: string[] = [
    'disability credit',
    'attendant or nursing home expense credit'
  ];
  ...
  constructor(public dataService: MspDataService) {
    this.showAttendantCareInfo =
      this.finAssistApp.applicantClaimForAttendantCareExpense ||
  ...
  ngOnInit() {
    this._showDisabilityInfo =
      ...
      this.dataService.finAssistApp.spouseEligibleForDisabilityCredit ===
        true ||
    ...
    this.showChildrenInfo =
      ...
      (!_.isNil(this.finAssistApp.claimedChildCareExpense_line214) &&
        this.finAssistApp.claimedChildCareExpense_line214 > 0) ||
      (!_.isNil(this.finAssistApp.reportedUCCBenefit_line117) &&
        this.finAssistApp.reportedUCCBenefit_line117 > 0);
  ...
  addReceipts(evt: any) {
    ...
    this.finAssistApp.attendantCareExpenseReceipts = this.finAssistApp.attendantCareExpenseReceipts.concat(
      evt
    );
  ...
  toggleClaimForSelfDisabilityCredit($event: Event): void {
    if (this.finAssistApp.applicantClaimForAttendantCareExpense) {
      ...
      this.finAssistApp.selfDisabilityCredit = !this.finAssistApp
  ...
  toggleClaimForSpouseDisabilityCredit($event: Event): void {
    if (
      this.finAssistApp.spouseClaimForAttendantCareExpense &&
      !this.finAssistApp.spouseEligibleForDisabilityCredit
    ) {
      $event.preventDefault();
      this.spouseClaimDisabilityCredit();
    } else {
      this.finAssistApp.spouseEligibleForDisabilityCredit = !this.finAssistApp
  ...
  ngDoCheck() {
    this.qualifiedForAssistance =
      this.finAssistApp.eligibility.adjustedNetIncome <=
      this.qualificationThreshhold;
    // fix for DEF-91
    this.requireAttendantCareReceipts =
      this.finAssistApp.applicantClaimForAttendantCareExpense ||
      this.finAssistApp.spouseClaimForAttendantCareExpense ||
      this.finAssistApp.childClaimForAttendantCareExpense;
  }
  ...
  applicantClaimForAttendantCareExpense($event) {
    if (
      ...
      this.finAssistApp.applicantClaimForAttendantCareExpense = !this
        .finAssistApp.applicantClaimForAttendantCareExpense;
    }
  }
  ...
  spouseClaimForAttendantCareExpense() {
    if (
      !this.finAssistApp.spouseClaimForAttendantCareExpense &&
      (this.finAssistApp.spouseDSPAmount_line125 ||
        this.finAssistApp.spouseEligibleForDisabilityCredit)
    ) {
      event.preventDefault();
      ...
      this.finAssistApp.spouseClaimForAttendantCareExpense = !this.finAssistApp
        .spouseClaimForAttendantCareExpense;
    }
  }

  childClaimForAttendantCareExpense() {
    this.finAssistApp.childClaimForAttendantCareExpense = !this.finAssistApp
      .childClaimForAttendantCareExpense;
  ...
  get getFinanialInfoSectionTitle() {
    if (!!this.userSelectedMostRecentTaxYear) {
      return this.lang('./en/index.js').checkEligibilityScreenTitle.replace(
  ...
  get userSelectedMostRecentTaxYear(): string {
    ...
    if (
      this.finAssistApp.assistYears &&
      this.finAssistApp.assistYears.length > 0
    ) {
      this.finAssistApp.assistYears.forEach(assistYear => {
